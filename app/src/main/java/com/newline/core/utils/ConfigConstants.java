...
//            .setBackgrounds(backgrounds)
              //.setProgressBarImage(ConfigConstants.sProgressDrawable)//进度条fresco:progressBarImage="@drawable/progress_bar"进度条
              //.setFadeDuration(3000)//fresco:fadeDuration="300"加载图片动画时间
...
//            .setOverlays(overlays)
              ...
              .setPlaceholderImage(ConfigConstants.sPlaceholderDrawable, ScalingUtils.ScaleType.FOCUS_CROP)//fresco:placeholderImageScaleType="fitCenter"占位图缩放类型
...
//			  .setLocalThumbnailPreviewsEnabled(true)//缩略图预览，影响图片显示速度（轻微）
        //imageRequest.setLowestPermittedRequestLevel(RequestLevel.FULL_FETCH);//请求经过缓存级别  BITMAP_MEMORY_CACHE，ENCODED_MEMORY_CACHE，DISK_CACHE，FULL_FETCH
        return imageRequest;
    }
...
//            .setMinDecodeIntervalMs(intervalMs)//最小解码间隔（分位单位）
    ...
    public static Drawable sProgressDrawable;


    @SuppressWarnings("deprecation")
    public static void init(final Resources resources) {
        if (sPlaceholderDrawable == null) {
            sPlaceholderDrawable = resources.getDrawable(R.drawable.nocontent);
        }
        if (sErrorDrawable == null) {
            sErrorDrawable = resources.getDrawable(R.drawable.nocontent);
        }
        if (sProgressDrawable == null) {
            sProgressDrawable = resources.getDrawable(R.drawable.nocontent);
        }
    }
